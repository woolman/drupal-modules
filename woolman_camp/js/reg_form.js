...
function tally() {
  ...
  $("#tab-nav li a").each(function() {
    ...
    if($(cid+' input:checkbox').length != 0) {
      if($(cid+' input:checkbox:checked').length != 0) {total += 1;}
      $(cid+' input:checkbox:checked').each(function() {
        weeks += parseInt($(this).val());
      ...
      if(weeks == 0) {
        ...
        $(this).find('.registered-for').html('Registered for: <strong>1</strong> week');
      }else {
        $(this).find('.registered-for').html('Registered for: <strong>'+weeks+'</strong> weeks');
      }
    }
  });
  if(total == 0) {
